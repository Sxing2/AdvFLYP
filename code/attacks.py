import torch
from code.utils import one_hot_embedding
import torch.nn.functional as F
import functools
from autoattack import AutoAttack
from code.func import clip_img_preprocessing, multiGPU_CLIP, multiGPU_CLIP_image_logits, image_text_cossim
...
def attack_pgd(args, prompter, model, model_text, model_image, add_prompter, criterion, X, target, alpha,
    ...
    # turn off model parameters temporarily
    tunable_param_names = []
    for n,p in model.module.named_parameters():
        if p.requires_grad: 
            tunable_param_names.append(n)
...
def attack_pgd_captions(args, prompter, model, model_text, model_image, add_prompter, X, captions, alpha,
               attack_iters, norm, restarts=1, early_stop=True, epsilon=0):
    delta = torch.zeros_like(X).cuda()
    if norm == "l_inf":
        delta.uniform_(-epsilon, epsilon)
    ...
    for iter in range(attack_iters):
        ...
        batch_logits = image_text_cossim(
            prompted_images, captions, model, prompt_token=prompt_token
        )
        labels = torch.arange(prompted_images.size(0), device=prompted_images.device)
        infoce_loss = (
...
def square_attack(model, images, target, text_tokens, epsilon=0):

    with torch.amp.autocast('cuda', enabled=False):
        forward_pass = functools.partial(
            multiGPU_CLIP_image_logits,
            model=model, text_tokens=text_tokens, prompter=None, add_prompter=None
        )
        adversary = AutoAttack(forward_pass, norm='Linf', eps=epsilon, 
                            version='custom', verbose=False, attacks_to_run=['square'])
